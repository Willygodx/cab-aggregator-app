...
public class RideServiceValidationImpl implements RideServiceValidation {
  ...
  public void validChangeRideStatus(Ride ride, RideStatusRequest ridesStatusRequest) {
    ...
    switch (currentStatus) {
      case CREATED ->
          checkStatus(currentStatus, newStatus, RideStatus.ACCEPTED);
      case ACCEPTED ->
          checkStatus(currentStatus, newStatus, RideStatus.ON_THE_WAY_TO_PASSENGER);
      case ON_THE_WAY_TO_PASSENGER ->
          checkStatus(currentStatus, newStatus, RideStatus.ON_THE_WAY_TO_DESTINATION);
      case ON_THE_WAY_TO_DESTINATION ->
          checkStatus(currentStatus, newStatus, RideStatus.FINISHED);
    }
  }
